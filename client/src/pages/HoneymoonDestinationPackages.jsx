...
const HoneymoonDestinationPackages = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/package/get-packages?category=Honeymoon&searchTerm=${encodeURIComponent(destination)}&limit=100`);
  ...
  const heroImage = useMemo(() => {
    return packages.length && packages[0]?.packageImages?.length ? packages[0].packageImages[0] : "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?q=80&w=1200&auto=format&fit=crop";
  }, [packages]);
  ...
  return (
    <div className="w-full">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        <button className="text-sm text-orange-600 hover:underline" onClick={() => navigate("/honeymoon")}>Back to Honeymoon Destinations</button>
...
export default HoneymoonDestinationPackages;