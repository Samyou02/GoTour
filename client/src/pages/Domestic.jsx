...
const Domestic = () => {
  ...
  const [domPackages, setDomPackages] = useState([]);

  useEffect(() => {
    const fetchDom = async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/package/get-packages?category=Domestic&limit=999`);
        const data = await res.json();
        setDomPackages(data?.packages || []);
        setLoading(false);
      } catch (e) {
        setLoading(false);
      }
  ...
  const destinations = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([dest, pkg]) => ({
      title: dest,
      image: (pkg.packageImages && pkg.packageImages[0]) || "https://images.unsplash.com/photo-1488085061387-422e29b40080?q=80&w=1200&auto=format&fit=crop",
      search: dest,
    }));
  }, [domPackages]);
    ...
    <div className="w-full bg-orange-50 min-h-[60vh]">
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="flex flex-col items-center gap-2 mt-4">
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold text-center">Domestic Destinations</h1>
          <p className="text-center text-sm text-gray-700 max-w-2xl">
            Choose these packages to explore Indian destinations privately with your loved ones.
          </p>
        </div>
          ...
          {!loading && destinations.map((g, i) => (
            <div key={i} className="bg-white rounded-xl shadow-md overflow-hidden">
              ...
              <div className="p-3">
                ...
                <Link
                  to={`/domestic-packages/${encodeURIComponent(g.search)}`}
