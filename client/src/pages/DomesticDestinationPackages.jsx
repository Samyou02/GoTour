...
const DomesticDestinationPackages = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await fetch(`/api/package/get-packages?category=Domestic&searchTerm=${encodeURIComponent(destination)}&limit=100`);
  ...
  const heroImage = useMemo(() => {
    return packages.length && packages[0]?.packageImages?.length ? packages[0].packageImages[0] : "https://images.unsplash.com/photo-1488085061387-422e29b40080?q=80&w=1200&auto=format&fit=crop";
  }, [packages]);
  ...
  return (
    <div className="w-full">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        <button className="text-sm text-orange-600 hover:underline" onClick={() => navigate("/domestic")}>Back to Domestic Destinations</button>
...
export default DomesticDestinationPackages;